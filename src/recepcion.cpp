...
void receiveData()
{
  ...
  if (index > 0)
  {
    ...
    index = 0;
    memcpy(&latitude, &buffer[index], sizeof(float));
    index += sizeof(float);
    memcpy(&longitude, &buffer[index], sizeof(float));
    index += sizeof(float);
    ...
    Serial.print(" Roll: ");
    Serial.print(roll, 2);
    Serial.print(" Presión del aire (Psi): ");
    ...
    Serial.print(" tiempo_envia: ");
    Serial.println(tiempo_envia, 5);
  }
  else
  {
